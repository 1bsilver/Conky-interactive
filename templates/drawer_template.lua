...
function draw_background(cr, w, h)
    ...
    cairo_stroke(cr)
    --Clock
    cairo_select_font_face (cr, "Anka/Coder", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD)
	...
	cairo_move_to(cr,w-145,76)
	cairo_show_text(cr,conky_parse('${exec date +%H:%M}'))
	--Date
    cairo_select_font_face (cr, "Anka/Coder", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
	...
	cpu_y = 180
	cpu_move = 30
	cairo_set_source_rgba(cr, r1, g, b1, t1)
    cairo_set_line_width(cr, 2)
    ...
    --CPU usage
--     cairo_select_font_face (cr, "Anka/Coder", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
--     cairo_set_source_rgba(cr, r1, g, b1, t_stats)
-- 	cairo_set_font_size(cr, 13)
-- 	ct = cairo_text_extents_t:create()
-- 	cairo_text_extents(cr,conky_parse('${cpu cpu0}%'),ct)
-- 	cairo_move_to(cr,350,199)
-- 	cairo_show_text(cr,conky_parse('${cpu cpu0}%'))
--

	uptime = conky_parse('${cpu cpu0}')
     cairo_set_source_rgba(cr, r1, g, b1, t_stats)
	cairo_set_font_size(cr, 13)
	ct = cairo_text_extents_t:create()
	cairo_text_extents(cr,uptime..'%',ct)
	cairo_move_to(cr,350,199)
    ...
    cairo_fill(cr)
      ...
      -- RAM used
      cairo_select_font_face (cr, "Anka/Coder", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
    cairo_set_source_rgba(cr, r2, g2, b2, t_stats)
	cairo_set_font_size(cr, 13)
	ct = cairo_text_extents_t:create()
	cairo_text_extents(cr,conky_parse('${mem}'),ct)
	cairo_move_to(cr,350,268.5)
    ...
    for i=0, 10 do
      ...
      cairo_select_font_face (cr, "Anka/Coder", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
    cairo_set_source_rgba(cr, r3, g3, b3, t_stats)
	cairo_set_font_size(cr, 13)
	ct = cairo_text_extents_t:create()
	cairo_text_extents(cr,conky_parse('${fs_used}'),ct)
	cairo_move_to(cr,350,337.5)
    ...
    --Battery
 	cairo_set_source_rgba(cr, r6, g6, b6, t4)
	cairo_set_line_width(cr, 2)
	--Battery structure
	bat_x = 69
	bat_y = 386
    ...
    --battery percent
      cairo_select_font_face (cr, "Anka/Coder", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
    cairo_set_source_rgba(cr, r6, g6, b6, t_stats)
	cairo_set_font_size(cr, 13)
	ct = cairo_text_extents_t:create()
	cairo_text_extents(cr,conky_parse('${battery_percent}%'),ct)
	cairo_move_to(cr,350,407)
	cairo_show_text(cr,conky_parse('${battery_percent}%'))
